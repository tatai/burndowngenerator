<?php
/* 
 * Yet another burndown online generator, http://www.burndowngenerator.com
 * Copyright (C) 2010 Francisco José Naranjo <fran.naranjo@gmail.com>
 * 
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as
 * published by the Free Software Foundation, either version 3 of the
 * License, or (at your option) any later version.
 * 
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 * 
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <http://www.gnu.org/licenses/>.
 */
require_once (dirname(__FILE__) . '/../test_startup.php');

class DrawLineTest extends PHPUnit_Framework_TestCase {
	/**
	 * 
	 * @var IPdf
	 */
	private $_pdf = null;
	
	/**
	 * 
	 * @var LineStyleChanger
	 */
	private $_styleChanger = null;

	public function setUp() {
		$this->_pdf = $this->getMock('IPdf');
		
		$this->_styleChanger = $this->getMock('LineStyleChanger');
	}

	/**
	 * @test
	 */
	public function createLineByCallingLineMethodOnPdfClass() {
		$from = $this->_randomPoint();
		$to = $this->_randomPoint();
		$line = new Line($from, $to);
		
		$this->_pdf->expects($this->once())->method('line')->with($from->x(), $from->y(), $to->x(), $to->y());
		
		$draw = new DrawLine($this->_pdf, $this->_styleChanger);
		$draw->draw($line);
	}

	private function _randomPoint() {
		return new Point(rand(1, 100), rand(1, 100));
	}

	/**
	 * @test
	 */
	public function ifLineStyleGivenItIsApplied() {
		$from = $this->_randomPoint();
		$to = $this->_randomPoint();
		$line = new Line($from, $to);
		
		$style = $this->getMock('ILineStyle');
		
		$this->_styleChanger->expects($this->once())->method('change');
		
		$draw = new DrawLine($this->_pdf, $this->_styleChanger);
		$draw->draw($line, $style);
	}
}

require_once (dirname(__FILE__) . '/../test_shutdown.php');